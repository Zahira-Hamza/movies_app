...
class _HomeTabState extends State<HomeTab> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    super.initState();
    _currentGenre = CategoryModel.categories[widget.selectedCategoryIndex].apiValue;
    _loadInitialData();
  }

  void _loadInitialData() {
    final cubit = context.read<MoviesCubit>();
    if (cubit.state is! MoviesLoaded || (cubit.state as MoviesLoaded).movies.isEmpty) {
      cubit.fetchMovies();
    }
  ...
  void didUpdateWidget(HomeTab oldWidget) {
    ...
    if (oldWidget.selectedCategoryIndex != widget.selectedCategoryIndex) {
      final newGenre = CategoryModel.categories[widget.selectedCategoryIndex].apiValue;
                            ...
                            ? ListView.separated(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  return CustomFilmPoster(
                                    ...
                                    rating: (genreMovie.rating ?? 0).toString(),
                                  );
                                },
